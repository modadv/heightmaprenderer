...
// E:\Projects\review-bgfx\bgfx\.build\win64_vs2022\bin\shadercDebug.exe -f cs_generate_smap.sc -o E:\Projects\review-bgfx\bgfx\examples\runtime\shaders\dx11\cs_generate_smap.bin --type compute -i E:\Projects\review-bgfx\bgfx\src -i E:\Projects\review-bgfx\bgfx\examples\runtime\shaders --varyingdef E:\Projects\review-bgfx\bgfx\examples\runtime\shaders\varying.def.sc --platform windows -p cs_5_0 --debug -O 0
...
static float4 u_params[2];
static float4 u_aspectParams;
uniform SamplerState u_heightmapSampler : register(s[0]); uniform Texture2D u_heightmapTexture : register(t[0]); static BgfxSampler2D u_heightmap = { u_heightmapSampler, u_heightmapTexture };
RWTexture2D<float4> u_smap_outputTexture : register(u[1]); static BgfxRWImage2D_rgba32f u_smap_output = { u_smap_outputTexture };
uniform float4 u_smapParams;
uniform float4 u_smapChunkParams;
[numthreads(32, 32, 1)]
void main( int3 gl_GlobalInvocationID : SV_DispatchThreadID )
{
int2 globalCoord = int2(gl_GlobalInvocationID.xy);
globalCoord += int2(u_smapChunkParams.xy);
float2 texSize = u_smapParams.xy;
if(any(greaterThanEqual(globalCoord, int2(texSize))))
return;
int i = globalCoord.x;
int j = globalCoord.y;
int w = int(texSize.x);
...
int j1 = max(0, j - 1);
int j2 = min(h - 1, j + 1);
float2 texCoord_l = (float2(i1, j) + 0.5) / texSize;
float2 texCoord_r = (float2(i2, j) + 0.5) / texSize;
float2 texCoord_b = (float2(i, j1) + 0.5) / texSize;
float2 texCoord_t = (float2(i, j2) + 0.5) / texSize;
float z_l = bgfxTexture2DLod(u_heightmap, texCoord_l, 0.0).r;
float z_r = bgfxTexture2DLod(u_heightmap, texCoord_r, 0.0).r;
float z_b = bgfxTexture2DLod(u_heightmap, texCoord_b, 0.0).r;
float z_t = bgfxTexture2DLod(u_heightmap, texCoord_t, 0.0).r;
float slope_x = float(w) * 0.5 * (z_r - z_l);
float slope_y = float(h) * 0.5 * (z_t - z_b);
imageStore(u_smap_output, globalCoord, float4(slope_x, slope_y, 0.0, 0.0));
}
