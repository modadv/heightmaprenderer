...
// E:\Projects\review-bgfx\bgfx\.build\win64_vs2022\bin\shadercDebug.exe -f fs_terrain_render.sc -o E:\Projects\review-bgfx\bgfx\examples\runtime\shaders\dx11\fs_terrain_render.bin --type fragment --platform windows -p ps_5_0 -i E:\Projects\review-bgfx\bgfx\src --debug -O 0
...
float2 s = bgfxTexture2D(u_SmapSampler, v_texcoord0).rg * u_params[0].x;
float3 n = normalize(float3(-s, 1.0));
float lightFactor = clamp(n.z, 0.1, 1.0);
float4 diffuseTexColor = bgfxTexture2D(u_DiffuseSampler, v_texcoord0);
float3 finalColor = diffuseTexColor.rgb * lightFactor;
bgfx_FragData0 = float4(finalColor, 1.0);
}
