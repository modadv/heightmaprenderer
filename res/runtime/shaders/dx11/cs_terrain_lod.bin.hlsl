...
uniform SamplerState u_DiffuseSamplerSampler : register(s[5]); uniform Texture2D u_DiffuseSamplerTexture : register(t[5]); static BgfxSampler2D u_DiffuseSampler = { u_DiffuseSamplerSampler, u_DiffuseSamplerTexture };
float dmap(float2 pos)
{
float2 uv;
uv.x = (pos.x + u_aspectParams.x) / (2.0 * u_aspectParams.x);
uv.y = (pos.y + u_aspectParams.y) / (2.0 * u_aspectParams.y);
return (bgfxTexture2DLod(u_DmapSampler, uv, 0.0).x) * u_params[0].x;
}
...
{
bool3 b = greaterThan(n, float3(0.0, 0.0, 0.0));
...
Buffer<uint> u_SubdBufferIn : register(t[8]);
RWBuffer<uint> u_CulledSubdBuffer : register(u[2]);
Buffer<float4> u_VertexBuffer : register(t[6]);
Buffer<uint> u_IndexBuffer : register(t[7]);
[numthreads(32u, 1u, 1u)]
...
float4x4 mvp = u_modelViewProj;
float4 bmin = min(min(v[0], v[1]), v[2]);
float4 bmax = max(max(v[0], v[1]), v[2]);
bmin.z = 0;
bmax.z = u_params[0].x;
if (u_params[0].z == 0
...
{
uint idx = 0;
InterlockedAdd(u_AtomicCounterBuffer[1], 2, idx);
